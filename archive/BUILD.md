...
しばらくの間、`news/YYYY-MM-DD.md` は**朝5:30の便（①②③...を新規作成）→夜18:30の便（④⑤⑥...を追記）**という形で、1日2回に分けて更新される。ファイルは1日1つのまま、`.issue`（号）も1日1号のまま変わらない——単に1日の中で記事数が3〜5件→6〜10件に増えていくだけなので、上記の変換ルールにそのまま従えばよい。夜の便がアーカイブを再構築する際は、`issue-sub`（記事見出し一覧・全N本の表記）が朝の時点より増えていることに注意する。
