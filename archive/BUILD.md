...
しばらくの間、`news/YYYY-MM-DD.md` は**深夜2:00の処理（朝分・①②③...を新規作成）→深夜3:00の処理（夜分・④⑤⑥...を追記）**という形で、1日2回に分けて更新される（2026-08-20以降、重い処理は深夜に移動。配信自体は従来通り5:40/18:40）。ファイルは1日1つのまま、`.issue`（号）も1日1号のまま変わらない——単に1日の中で記事数が3〜5件→6〜10件に増えていくだけなので、上記の変換ルールにそのまま従えばよい。夜分の処理がアーカイブを再構築する際は、`issue-sub`（記事見出し一覧・全N本の表記）が朝分の時点より増えていることに注意する。各処理は自分が追加したバッチの複製を `news/cache/YYYY-MM-DD-am.md` または `news/cache/YYYY-MM-DD-pm.md` としても保存し、後で配信トリガーがそれを読んで（Web検索せずに）そのまま配信する。
